...
async function buildOffline() {
  try {
    ...
    execSync('astro build --config config/astro.offline.config.mjs', {
      stdio: 'inherit',
      env: { ...process.env, PUBLIC_OFFLINE_PATHS: 'true' }
    })

    // Copy assets to offline build
    await copyAssetsForOffline()

    // Copy CSS and JS files
    await copyCSSJSForOffline()
...
async function formatHTMLFiles() {
  try {
    ...
    log(`Formatted ${files.length} HTML files`, 'success', 'BUILD-OFF')
  } catch (error) {
    log(`HTML formatting error: ${error.message}`, 'error', 'BUILD-OFF')
 ...
 */
async function copyAssetsForOffline() {
  try {
    ...
    // Copy favicon files to root of offline build
    ...
    log(`Asset copy failed: ${error.message}`, 'error', 'BUILD-OFF')
    throw error
  }
}
 ...
 */
async function copyCSSJSForOffline() {
  try {
    // Copy CSS files
    ...
    log(`CSS/JS copy failed: ${error.message}`, 'error', 'BUILD-OFF')
    throw error
  }
}
...
async function postProcessHTMLFiles() {
  try {
    ...
    htmlFiles.forEach((filePath) => {
      ...
      // Ensure proper offline paths for deeper nested files
    ...
    log(`Post-processed ${htmlFiles.length} HTML files for offline paths`, 'info', 'BUILD-OFF')
  } catch (error) {
    log(`HTML post-processing failed: ${error.message}`, 'error', 'BUILD-OFF')
    throw error
  }
}
